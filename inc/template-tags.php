...
if ( ! function_exists( 'archd_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 *
	 * @author WDS
	 */
	function archd_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'archd' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: the post author */
			esc_html_x( 'by %s', 'post author', 'archd' ),
...
if ( ! function_exists( 'archd_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 *
	 * @author WDS
	 */
	function archd_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'archd' ) );
			if ( $categories_list && archd_categorized_blog() ) {
				/* translators: the post category */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'archd' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'archd' ) );
			if ( $tags_list ) {
				/* translators: the post tags */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'archd' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'archd' ), esc_html__( '1 Comment', 'archd' ), esc_html__( '% Comments', 'archd' ) );
			echo '</span>';
		}
 ...
 * @author WDS
 */
function archd_display_svg( $args = array() ) {
	echo archd_get_svg( $args ); // WPCS XSS Ok.
}
 ...
 * @return string
 */
function archd_get_svg( $args = array() ) {

	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'archd' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'archd' );
	}
	...
	<svg
	<?php
		echo archd_get_the_content( $height ); // WPCS XSS OK.
		echo archd_get_the_content( $width ); // WPCS XSS OK.
		echo archd_get_the_content( $fill ); // WPCS XSS OK.
	?>
		class="icon icon-<?php echo esc_attr( $args['icon'] ); ?>"
	<?php
		echo archd_get_the_content( $aria_hidden ); // WPCS XSS OK.
		echo archd_get_the_content( $aria_labelledby ); // WPCS XSS OK.
	?>
		role="img">
 ...
 * @return string
 */
function archd_get_the_title( $args = array() ) {
 ...
 * @return string
 */
function archd_get_the_excerpt( $args = array() ) {
 ...
 * @return bool
 */
function archd_display_copyright_text() {

	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'archd_copyright_text' );
	...
	echo archd_get_the_content( do_shortcode( $copyright_text ) ); // phpcs: xss ok.
}
 ...
 * @return string
 */
function archd_get_twitter_share_url() {
 ...
 * @return string
 */
function archd_get_facebook_share_url() {
 ...
 * @return string
 */
function archd_get_linkedin_share_url() {
	return add_query_arg(
		array(
 ...
 * @author WDS
 */
function archd_display_social_network_links() {
	...
	<ul class="social-icons">
		...
		foreach ( $social_networks as $network ) :
			...
			$network_url = get_theme_mod( 'archd_' . $network . '_link' );
			...
			?>
				<li class="social-icon <?php echo esc_attr( $network ); ?>">
					<a href="<?php echo esc_url( $network_url ); ?>">
						...
						<?php
							echo /* translators: the social network name */ sprintf( esc_html( 'Link to %s', 'archd' ), ucwords( esc_html( $network ) ) ); // WPCS: XSS OK.
						?>
						</span>
					</a>
				</li><!-- .social-icon -->
 ...
 */
function archd_display_card( $args = array() ) {
	...
	<div class="<?php echo esc_attr( $args['class'] ); ?> card">
		...
		<a href="<?php echo esc_url( $args['url'] ); ?>" tabindex="-1">
			...
			<?php else : ?>
				<img src="<?php echo esc_url( get_stylesheet_directory_uri() ); ?>/assets/images/placeholder.png" class="card-image" loading="lazy" alt="<?php echo sprintf( esc_attr( 'Featured image for %s', 'archd' ), esc_attr( $args['title'] ) ); ?>">
			<?php endif; ?>
		</a>
		...
		<?php if ( $args['url'] ) : ?>
			<a class="button button-card" href="<?php echo esc_url( $args['url'] ); ?>"><?php esc_html_e( 'Read More', 'archd' ); ?></a>
		<?php endif; ?>
 ...
 * @return string
 */
function archd_display_header_button() {
	...
	$button_url  = get_theme_mod( 'archd_header_button_url' );
	$button_text = get_theme_mod( 'archd_header_button_text' );
	?>
	<div class="site-header-action">
		...
		?>
			<a href="<?php echo esc_url( $button_url ); ?>" class="button button-link"><?php echo esc_html( $button_text ?: __( 'More Information', 'archd' ) ); ?></a>
		<?php else : ?>
			<button type="button" class="cta-button" aria-expanded="false" aria-label="<?php esc_html_e( 'Search', 'archd' ); ?>">
				<?php esc_html_e( 'Search', 'archd' ); ?>
			</button>
 ...
 * @author Corey Collins
 */
function archd_display_numeric_pagination( $args = array() ) {
	...
	<nav class="pagination-container container" aria-label="<?php esc_html_e( 'numeric pagination', 'archd' ); ?>">
 ...
 * @author Corey Collins
 */
function archd_display_mobile_menu() {
	...
	<nav class="off-canvas-container" aria-label="<?php esc_html_e( 'Mobile Menu', 'archd' ); ?>" aria-hidden="true" tabindex="-1">
		<button type="button" class="off-canvas-close" aria-label="<?php esc_html_e( 'Close Menu', 'archd' ); ?>">
 ...
 * @return bool
 */
function archd_has_array_key( $key, $array = array() ) {
