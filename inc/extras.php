 ...
 */
function archd_categorized_blog() {

	$category_count = get_transient( 'archd_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'archd_categories', $category_count );
	}
 ...
 */
function archd_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function archd_get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'archd' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'archd' ) );
	}
	...
	$alt = apply_filters( 'archd_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'archd' ) );
 ...
 * @return string
 */
function archd_copyright_year( $atts ) {
...
add_shortcode( 'archd_copyright_year', 'archd_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	 ...
	 * @since  NEXT
	 */
	function archd_move_yoast_to_bottom() {
		return 'low';
	}
	add_filter( 'wpseo_metabox_prio', 'archd_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function archd_get_the_content( $content ) {
...
add_filter( 'the_content', 'archd_get_the_content', 20 );
 ...
 */
function archd_get_theme_colors_gutenberg() {
