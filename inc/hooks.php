 ...
 * @return array
 */
function archd_body_classes( $classes ) {
...
add_filter( 'body_class', 'archd_body_classes' );

/**
 * Flush out the transients used in archd_categorized_blog.
 *
 * @author WDS
 * @return string
 */
function archd_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'archd_categories' );
}
add_action( 'delete_category', 'archd_category_transient_flusher' );
add_action( 'save_post', 'archd_category_transient_flusher' );
 ...
 * @return string
 */
function archd_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'archd' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'archd_content_more_link' );
 ...
 * @return string
 */
function archd_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'archd' ) );
}
add_filter( 'excerpt_more', 'archd_excerpt_more' );
 ...
 * @return array
 */
function archd_custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'archd_custom_mime_types' );
...
add_shortcode( 'svg', 'archd_display_svg' );
 ...
 * @return string
 */
function archd_display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( 'archd_header_scripts' );
	...
	echo archd_get_the_content( $scripts ); // WPCS XSS OK.
}
add_action( 'wp_head', 'archd_display_customizer_header_scripts', 999 );
 ...
 * @return string
 */
function archd_display_customizer_footer_scripts() {

	// Check for footer scripts.
	$scripts = get_theme_mod( 'archd_footer_scripts' );
...
add_action( 'wp_footer', 'archd_display_customizer_footer_scripts', 999 );
...
function archd_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'archd' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'archd' ) : esc_html__( 'tagged with', 'archd' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'archd' ), $specify, $term_name );
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'archd' ), $search_term );
	}
...
add_action( 'wp_head', 'archd_add_og_tags' );
 ...
 * @author Corey Collins
 */
function archd_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'archd_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function archd_remove_empty_p_tags_from_content( $content ) {
...
add_filter( 'the_content', 'archd_remove_empty_p_tags_from_content' );
 ...
 * @author Corey Collins
 */
function archd_add_chrome_lazy_loading_to_images( $attr ) {

	$attr['loading'] = 'lazy';

	return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'archd_add_chrome_lazy_loading_to_images' );
 ...
 * @author Corey Collins
 */
function archd_allow_img_attributes( $allowedposttags ) {
...
add_filter( 'wp_kses_allowed_html', 'archd_allow_img_attributes', 1 );
 ...
 * @author Corey Collins
 */
function archd_add_lazy_loading_to_image_block( $block_content, $block ) {
...
add_filter( 'render_block', 'archd_add_lazy_loading_to_image_block', 10, 2 );
