 ...
 */
function archd_display_block_options( $args = array() ) {
	...
	// Get the block ID.
	$block_id = archd_get_block_id( $args['block'] );
	...
	$has_show_overlay = archd_has_array_key( 'show_overlay', $background_options ) && true === $background_options['show_overlay'] ? ' has-overlay' : '';
	...
	if ( $args['background_type'] ) {
		...
		if ( 'video' === $args['background_type'] ) {
			...
			$background_alt = $background_title ? sprintf( esc_attr( 'Video Background of %s', 'archd' ), esc_attr( $background_options['background_video_title'] ) ) : __( 'Video Background', 'archd' );
			...
			?>
				...
				<button class="video-toggle"><span class="screen-reader-text"><?php esc_html_e( 'Toggle video playback', 'archd' ); ?></span></button>
			<?php
 ...
 * @return bool
 */
function archd_has_block_expired( $args = array() ) {
 ...
 */
function archd_acf_flexible_content_layout_title( $block_title, $field, $layout, $i ) {
	...
	if ( 'none' !== $background ) {
		...
		$type = archd_return_flexible_content_layout_value( $background_type );
		...
		if ( 'video' === $background_type ) {
			$block_heading .= '<div style="font-size: 30px; height: 26px; width: 30px;" class="dashicons dashicons-format-video acf-flexible-title-image"><span class="screen-reader-text">' . esc_html__( 'Video', 'archd' ) . '</span></div>';
		}
	}
	...
	if ( archd_has_block_expired(
		...
		$expired .= '<span style="color: red;">&nbsp;(' . esc_html__( 'expired', 'archd' ) . ')</span>';
	}
...
add_filter( 'acf/fields/flexible_content/layout_title/name=content_blocks', 'archd_acf_flexible_content_layout_title', 10, 4 );
 ...
 */
function archd_return_flexible_content_layout_value( $type ) {
...
if ( function_exists( 'archd_acf_flexible_content_layout_title' ) ) {

	/**
	 * Set Admin Styles for Flexible Content Layout Image/Title in archd_acf_flexible_content_layout_title().
	 *
	 * @author WDS
	 */
	function archd_flexible_content_layout_title_acf_admin_head() {
	?>
	<style type="text/css">
	...
	add_action( 'acf/input/admin_head', 'archd_flexible_content_layout_title_acf_admin_head' );
}

/**
 * Load colors dynamically into select field from archd_get_theme_colors()
 *
 * @author WDS
 ...
 */
function archd_acf_load_color_picker_field_choices( $field ) {

	// Reset choices.
	$field['choices'] = array();

	// Grab our colors array.
	$colors = archd_get_theme_colors();
...
add_filter( 'acf/load_field/name=color_picker', 'archd_acf_load_color_picker_field_choices' );
 ...
 */
function archd_get_theme_colors() {
	return array(
		esc_html__( 'Alto', 'archd' )           => '#ddd',
		esc_html__( 'Black', 'archd' )          => '#000',
		esc_html__( 'Blue', 'archd' )           => '#21759b',
		esc_html__( 'Cod Gray', 'archd' )       => '#111',
		esc_html__( 'Dove Gray', 'archd' )      => '#666',
		esc_html__( 'Gallery', 'archd' )        => '#eee',
		esc_html__( 'Gray', 'archd' )           => '#808080',
		esc_html__( 'Gray Alt', 'archd' )       => '#929292',
		esc_html__( 'Light Yellow', 'archd' )   => '#fff9c0',
		esc_html__( 'Mineshaft', 'archd' )      => '#333',
		esc_html__( 'Silver', 'archd' )         => '#ccc',
		esc_html__( 'Silver Chalice', 'archd' ) => '#aaa',
		esc_html__( 'White', 'archd' )          => '#fff',
		esc_html__( 'Whitesmoke', 'archd' )     => '#f1f1f1',
	);
}
 ...
 * @return void
 */
function archd_display_hero_heading( $block_title ) {
 ...
 * @since NEXT
 */
function archd_display_link( $args = array() ) {
	echo archd_get_link( $args ); // WPCS: XSS Ok.
}
 ...
 */
function archd_get_link( $args = array() ) {
	...
	$title = archd_has_array_key( 'title', $button_array ) ? $button_array['title'] : esc_html__( 'Read More', 'archd' );

	// Get url.
	$url = archd_has_array_key( 'url', $button_array ) ? $button_array['url'] : '';

	// Get target, else default internal.
	$target = archd_has_array_key( 'target', $button_array ) ? $button_array['target'] : '_self';
	?>
