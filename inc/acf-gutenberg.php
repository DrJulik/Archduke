...
function archd_acf_init() {
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-accordion',
			'title'           => esc_html__( 'Accordion', 'archd' ),
			'description'     => esc_html__( 'A custom set of collapsable accordion items.', 'archd' ),
			'render_callback' => 'archd_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'sort',
			'keywords'        => array( 'accordion', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'archd_acf_enqueue_accordion_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'           => esc_html__( 'Accordion Block Title', 'archd' ),
						'text'            => esc_html__( 'Accordion Block Content', 'archd' ),
						'accordion_items' => array(
							'0' => array(
								'accordion_title' => esc_html__( 'Accordion Item Title', 'archd' ),
								'accordion_text'  => esc_html__( 'Accordion Item Content', 'archd' ),
							),
						),
					),
				),
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-carousel',
			'title'           => esc_html__( 'Carousel', 'archd' ),
			'description'     => esc_html__( 'A carousel with a call to action for each slide.', 'archd' ),
			'render_callback' => 'archd_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
			'keywords'        => array( 'carousel', 'slider', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'archd_acf_enqueue_carousel_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-cta',
			'title'           => esc_html__( 'Call To Action', 'archd' ),
			'description'     => esc_html__( 'A call to action block.', 'archd' ),
			'render_callback' => 'archd_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'megaphone',
			'keywords'        => array( 'call to action', 'cta', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'archd_acf_enqueue_backend_block_styles',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'       => esc_html__( 'Call To Action Title', 'archd' ),
						'text'        => esc_html__( 'Call To Action Text', 'archd' ),
						'button_link' => array(
							'title' => esc_html__( 'Learn More', 'archd' ),
							'url'   => '#',
						),
					),
				),
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Fifty/Fifty Block', 'archd' ),
			'description'     => esc_html__( 'A split-content block with text and/or media.', 'archd' ),
			'render_callback' => 'archd_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'editor-table',
			...
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'text_primary' => esc_html__( 'Primary Text content', 'archd' ),
						'media_right'  => array(
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-hero',
			'title'           => esc_html__( 'Hero Block', 'archd' ),
			'description'     => esc_html__( 'A hero with an optional call to action.', 'archd' ),
			'render_callback' => 'archd_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Recent Posts Block', 'archd' ),
			'description'     => esc_html__( 'A set of recent posts displayed by category and/or tag.', 'archd' ),
			'render_callback' => 'archd_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Related Posts Block', 'archd' ),
			'description'     => esc_html__( 'A set of manually selected posts.', 'archd' ),
			'render_callback' => 'archd_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
...
add_action( 'acf/init', 'archd_acf_init' );
 ...
 * @author Corey Collins
 */
function archd_acf_block_registration_callback( $block ) {
	...
	if ( ! is_admin() && archd_has_block_expired(
		array(
	...
	archd_display_expired_block_message();
 ...
 * @author Corey Collins
 */
function archd_add_block_categories( $categories, $post ) {
	...
	return array_merge(
		...
		array(
			array(
				'slug'  => 'wds-blocks',
				'title' => esc_html__( 'WDS Blocks', 'archd' ),
			),
		)
	);
}
add_filter( 'block_categories', 'archd_add_block_categories', 10, 2 );
 ...
 * @author Corey Collins
 */
function archd_acf_enqueue_backend_block_styles() {
 ...
 * @author Corey Collins
 */
function archd_acf_enqueue_carousel_scripts() {

	if ( ! is_admin() ) {
		return;
	}

	archd_acf_enqueue_backend_block_styles();
 ...
 * @author Corey Collins
 */
function archd_acf_enqueue_accordion_scripts() {
 ...
 * @author Corey Collins
 */
function archd_get_block_alignment( $block ) {
 ...
 * @author Corey Collins
 */
function archd_get_block_classes( $block ) {
	...
	$classes  = '';
	$classes  = archd_get_block_expired_class();
 ...
 * @author Corey Collins
 */
function archd_get_block_expired_class() {
 ...
 * @author Corey Collins
 */
function archd_display_expired_block_message() {

	if ( ! archd_get_block_expired_class() ) {
		return;
	}

	?>
	<div class="block-expired-message">
		<span class="block-expired-text"><?php esc_html_e( 'Your block has expired. Please change or remove the Start and End dates under Other Options to display your block on the frontend.', 'archd' ); ?></span>
	</div>
	<?php
}
 ...
 * @author Corey Collins
 */
function archd_get_block_id( $block ) {
 ...
 * @author Corey Collins
 */
function archd_acf_gutenberg_display_admin_default_carousel( $block ) {
	...
	for ( $slides = 0; $slides < 2; $slides++ ) :
		...
		<section class="slide">
			<div class="slide-content container">
				<h2 class="slide-title"><?php esc_html_e( 'Slide Title', 'archd' ); ?></h2>
				<p class="slide-description"><?php esc_html_e( 'Slide Content', 'archd' ); ?></p>
			</div>
		</section>
	<?php
	endfor;
